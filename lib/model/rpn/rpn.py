...
class _RPN(nn.Module):
    ...
    def forward(self, base_feat, im_info, gt_boxes, num_boxes, attentions=None, angle_attentions=None):
        ...
        if attentions is not None:
            ...
            gated_feat = rpn_conv1 * (1 + sem_gate)
            if angle_attentions is not None:
                ang_vec = F.normalize(angle_attentions.mean(0), dim=0)
...
class _RPN_out_pred_label(nn.Module):
    ...
    def forward(self, base_feat, im_info, gt_boxes, num_boxes, attentions=None, angle_attentions=None):
        ...
        # return feature map after convrelu layer
        rpn_conv1 = F.relu(self.RPN_Conv(base_feat), inplace=True)
        # dual-guidance channel modulation with residual to avoid feature collapse
        gated_feat = rpn_conv1
        if attentions is not None:
            sem_vec = F.normalize(attentions.mean(0), dim=0)
            sem_gate = torch.sigmoid(self.sem_linear(sem_vec)).view(1, -1, 1, 1)
