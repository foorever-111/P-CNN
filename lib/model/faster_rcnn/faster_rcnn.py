    ...
    def forward(self, im_data_list, im_info_list, gt_boxes_list, num_boxes_list, average_shot=None,
        ...
        if self.meta_train and self.use_meta_head:
            ...
            for b in range(batch_size):
                ...
                for i in range(sem_att.size(0)):  # attentions len(attentions/meta_classes)*2048
                    if prn_cls[i].numpy()[0] + 1 not in unique_labels:
                        ...
                        continue
                    fused_attention = torch.sigmoid(getattr(self, 'w_sem', 1.0) * sem_att[i])
                    if ang_att is not None:
                        fused_attention = fused_attention + torch.sigmoid(getattr(self, 'w_geo', 1.0) * ang_att[i])
            ...
            for i in range(len(mean_sem)):
                ...
                fused_attention = torch.sigmoid(getattr(self, 'w_sem', 1.0) * mean_attentions)
                if mean_ang is not None:
                    fused_attention = fused_attention + torch.sigmoid(getattr(self, 'w_geo', 1.0) * mean_ang[i])
